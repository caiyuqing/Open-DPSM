...
class pupil_prediction:
    ...
    def preparingDataForModeling(self, num, movieList):
        ...
        if self.useEtData:
            filename_csv = self.eyetrackingDir + f"\\{self.subject}\\" + movieNum +".csv"
    ...
    def pupil_prediction(self):
        ...
        print(f"Modeling in progress for {self.subject}. Please wait...")
    ...
    def regularization(self, plot_reg = False):
        ...
        for num in range(len(self.movieList)):
            ...
            for i in range(luminanceMagnPerImPartTime.shape[0]):
                ...
                if self.pupil_zscore:
                    ...
                    y_pred_onewedge = contrastConv_onewedge*amplitudeWeightContrast+lumConv_onewedge
        ...
        if self.regularizationType == 'ridge':
            ...
            if self.pupil_zscore:
                ...
                self.y_predAll = self.zscore(ridge.predict(X))
                self.lumConvAll = self.zscore(ridge.predict(y_pred_lum_all.T))
                self.contrastConvAll = self.zscore(ridge.predict(y_pred_contrast_all.T))
            else:
    ...
    def modelContrast(self, params):
        ...
        for num in range(len(self.movieList)):
            ...
            lumConv = self.convolve_cum(rf_lum, lumData)
            contrastConv = self.convolve_spike(rf_contrast, contrastData)
            lumConv[~np.isfinite(lumConv)] =0
            contrastConv[~np.isfinite(contrastConv)] =0
            if np.nanstd(lumConv) !=0 and np.nanstd(contrastConv)!=0:
                if self.pupil_zscore:
                    lumConv = self.zscore(lumConv)
                    contrastConv = self.zscore(contrastConv)
                    y_pred = self.zscore(contrastConv*amplitudeWeightContrast+lumConv)
                else:
            ...
            lumConvAll = np.concatenate([lumConvAll,lumConv])
    ...
    def save_modelResults_permovie(self, subject, movie):
        ...
        if self.useEtData:
            os.chdir(self.outputDir + "\\Modeling result")
    ...
    def save_modelResults(self,subject):
        ...
        if 'modelContrast' not in self.modelResultDict[subject].keys():
            self.modelResultDict[subject]["modelContrast"] = {}
        self.modelResultDict[subject]["modelContrast"]["predAll"] = self.y_predAll
        self.modelResultDict[subject]["modelContrast"]["lumConvAll"] = self.lumConvAll
        self.modelResultDict[subject]["modelContrast"]["contrastConvAll"] = self.contrastConvAll
    ...
    def save_modelResults_reg(self, subject):
        ...
        self.modelResultDict[subject]["modelRegularization"]["predAll"] = self.y_predAll
        self.modelResultDict[subject]["modelRegularization"]["lumConvAll"] = self.lumConvAll
        self.modelResultDict[subject]["modelRegularization"]["contrastConvAll"] = self.contrastConvAll
