...
class tkfunctions:
    def __init__(self):
        ...
        self.scrGamFac = 2.2
        # What is the ratio between gaze-centered coordinate system and the screen (this is no longer needed)
        # self.A = 2
    ...
    def buttonFunc_check_data(self):
        ...
        if hasattr(self, "filename_csv"):
            ...
            df_eyetracking = pd.read_csv(self.filename_csv, index_col=None, header = None)
        ...
        if not hasattr(self, "filename_csv"):
            ...
            if self.eyetracking_duration != self.video_duration:
                tk.Label(self.window, text = f"Eyetracking data and video data do not have the (exact) same duration (Movie: {self.video_duration}s, eye tracking: {self.eyetracking_duration.round(2)}s).\nWhat do you want to do?",  font = ("Arial", 10), justify = tk.RIGHT,anchor = "e", width = 100,  bg = "#d9d9d9").grid(column = 0, row = 1, pady = 10)    
            ...
            tk.Label(self.window, text = f"What is the physical width of the screen? (in cm)",font = ("Arial", 10), justify = tk.RIGHT,anchor = "e", width = 100,  bg = "#d9d9d9").grid(column = 0, row = 7, pady = 10)
            self.entry_screenwidthcm = ttk.Entry(self.window)
            self.entry_screenwidthcm.insert(0, "145") # this is our data
            self.entry_screenwidthcm.grid(column = 1, row = 7)
            # distance between screen and eye
            tk.Label(self.window, text = f"What is the distance between the eye and the monitor? (in cm)",font = ("Arial", 10), justify = tk.RIGHT,anchor = "e", width = 100,  bg = "#d9d9d9").grid(column = 0, row = 8, pady = 10)
            self.entry_screeneyecm = ttk.Entry(self.window)
            self.entry_screeneyecm.insert(0, "75") # this is our data
            self.entry_screeneyecm.grid(column = 1, row = 8)
        ...
        self.exitButton.grid(column = 1, row = 11)
    ...
    def buttonFunc_feature_extraction(self):    
        ...
        # if events are not extracted, extracting events
            ...
            label_info = tk.Label(self.window, text = "Please wait...", fg = "green")
            label_info.grid(column = 0, row = 14)
            # event extraction object
            videoWidthCM = self.videoRealWidth / (self.eyetracking_width/self.screen_width)
            videoWidthDeg =math.degrees(math.atan(videoWidthCM/2/self.eye_to_screen))*2
            # visual angle of the regional weight map 
            degVF = videoWidthDeg *self.degVF_param
    ...
    def buttonFunc_modeling(self):
        ...
        if hasattr(self, "filename_csv"):
            ...
            label_info_plot = tk.Label(self.window, text = "Plotting can take a few second!", fg = "Black")
            label_info_plot.grid(column = 0, row = 18)
        else:
