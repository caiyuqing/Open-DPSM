...
class interactive_plot:
    def plot(self):
        ...
        sampledTimeStamps = self.modelDataDict[self.subject][self.movieName]["timeStamps"]
        sampledpupilData= self.modelDataDict[self.subject][self.movieName]["pupil"] 
        sampledgazexData = self.modelDataDict[self.subject][self.movieName]["gazex"] 
        sampledgazeyData = self.modelDataDict[self.subject][self.movieName]["gazey"] 
        lumData = self.modelDataDict[self.subject][self.movieName]["lumData"] 
        contrastData = self.modelDataDict[self.subject][self.movieName]["contrastData"] 
        y_pred = self.modelResultDict[self.subject]["modelContrast"][self.movieName]["pred"] 
        lumConv = self.modelResultDict[self.subject]["modelContrast"][self.movieName]["lumConv"] 
        contrastConv = self.modelResultDict[self.subject]["modelContrast"][self.movieName]["contrastConv"] 
        pupil_baseline = self.df_pupil_lum[(self.df_pupil_lum["subject"] == self.subject) & (self.df_pupil_lum["movie"] == self.movieName)]["pupil_pred_mixed"].values[0]
        if not self.gazecentered:
            # set all the gaze data to the center of the screen
            sampledgazexData = [self.eyetracking_width/2] * len(sampledgazexData)
            sampledgazeyData = [self.eyetracking_height/2] * len(sampledgazeyData)
        ...
        self.top_interactive_figure.geometry("1100x700+0+0")
        self.top_interactive_figure.resizable(False, False)
        
    
        init_frame = 3
        # Create the figure and the line that we will manipulate
        os.chdir(self.movieDir)
        movieFile = [file for file in os.listdir(self.movieDir) if self.movieName in file][0]
        video = cv2.VideoCapture(movieFile)
        self.sampledFps = video.get(cv2.CAP_PROP_FPS)
        ...
        if self.videoScreenSameRatio and self.videoStretched:
            ...
            screen = np.zeros((int(self.eyetracking_height), int(self.eyetracking_width),np.shape(frame)[2]),dtype = 'uint8')
        ...
        video_x = self.eyetracking_width
        video_y = self.eyetracking_height
        ...
        f = plt.figure(figsize = (10,10),constrained_layout = False)
        ...
        axes[0].set_title("Movie and gaze position\n(Red dot: gaze position)", fontsize = fontsize, loc = "center")
        ...
        finalImgWidth = self.finalImgWidth
        finalImgHeight = self.finalImgHeight
        ...
        center, = axes[1].plot((finalImg.shape[1]-1)/2,(finalImg.shape[0]-1)/2, 'ro'),
        axes[1].set_title(f"Extracted image",fontsize= fontsize, loc = "center")
        axes[1].set_xticks([])
        axes[1].set_yticks([])
        # create legend for marker
        # gaze_marker = mlines.Line2D([], [], color='red', marker='o', linestyle='None',
        #                           markersize=8, label='Gaze position')
        # if self.gazecentered:
        #     center_marker = mlines.Line2D([], [], color='blue', marker='P', linestyle='None',
        #                               markersize=8, label='Center of the new coordinate system')
        # else:
        #     center_marker = mlines.Line2D([], [], color='blue', marker='P', linestyle='None',
        #                               markersize=8, label='Center of screen')
        # axes[1].legend(handles=[gaze_marker, center_marker],bbox_to_anchor=(1.7, 0.5),fontsize = 10, frameon = False)
        #
        ...
        if self.gazecentered:
            ...
            axes[2].set_title("Gaze position assumed to be at the screen center")
        ...
        axes[3].set_ylim([min(lumData),max(contrastData)])
        ...
        tk.Button(self.top_buttom, text='Update figure', command=buttomFunc_updateFig2).grid(row=10, column =1, sticky=tk.W+tk.E,columnspan = 2)
        ...
        def buttomFunc_saveFig():
            ...
            if choice == "Save gaze":
                ...
                f.savefig(f'{self.movieName}_{self.subject}_Fig_gaze.png', bbox_inches=extent.expanded(1.1, 1.3))
                showinfo(title='',message=f'{self.subject}_Fig_gaze.png is saved')
            elif choice == 'Save visual events':
                extent = axes[3].get_window_extent().transformed(f.dpi_scale_trans.inverted())
                f.savefig(f'{self.movieName}_{self.subject}_Fig_feature.png', bbox_inches=extent.expanded(1.1, 1.3))
                showinfo(title='',message=f'{self.movieName}_{self.subject}_Fig_feature.png is saved')
    
            elif choice == "Save model prediction":
                extent = axes[4].get_window_extent().transformed(f.dpi_scale_trans.inverted())
                f.savefig(f'{self.movieName}_{self.subject}_Fig_model.png', bbox_inches=extent.expanded(1.1, 1.3))
                showinfo(title='',message=f'{self.movieName}_{self.subject}_Fig_model.png is saved')
    
            elif choice == "Save all":
                f.savefig(f'{self.movieName}_{self.subject}_Fig_all.png')
                showinfo(title='',message=f'{self.movieName}_{self.subject}_Fig_all.png is saved')
        ...
        choicebox_figure.grid(column = 1, row = 11,sticky=tk.E+tk.W,pady = (30,0))
        #choice_figure.trace('w', callback_figure)        
        tk.Button(self.top_buttom, text='Save fig', command = buttomFunc_saveFig).grid(row=11, column =2, sticky=tk.W+tk.E,pady = (30,0))
        
        tk.Button(self.top_buttom, text='Back', command = self.close_top_figure).grid(row=12, column =1, sticky=tk.W+tk.S+tk.E)
        tk.Button(self.top_buttom, text='Exit', command = self.close).grid(row=12, column =2, sticky=tk.W+tk.N+tk.S+tk.E)
        ...
        def updateValue(event):
            ...
            original_movie.set_data(screen[:,:,::-1])
    ...
    def plot_NoEyetracking(self):
        ...
        os.chdir(foldername)
        #Create dictionaries to save results
        if os.path.exists("modelDataDict_noETdata.pickle"):
            with open("modelDataDict_noETdata.pickle", "rb") as handle:
                modelDataDict = pickle.load(handle)
                handle.close() 
        else:
            modelDataDict = {}
                
        if os.path.exists("modelResultDict_noETdata.pickle"):
            with open("modelResultDict_noETdata.pickle", "rb") as handle:
                modelResultDict = pickle.load(handle)
                handle.close() 
        self.modelResultDict = modelResultDict
        self.modelDataDict= modelDataDict
        ...
        lumData = self.modelDataDict[self.subjectName][self.movieName]["lumData"] 
        contrastData = self.modelDataDict[self.subjectName][self.movieName]["contrastData"] 
        y_pred = self.modelResultDict[self.subjectName]["modelContrast"][self.movieName]["pred"] 
        lumConv = self.modelResultDict[self.subjectName]["modelContrast"][self.movieName]["lumConv"] 
        contrastConv = self.modelResultDict[self.subjectName]["modelContrast"][self.movieName]["contrastConv"] 
        ...
        if self.useApp:
            ...
            self.window = tk.Tk()        
        self.top_interactive_figure = tk.Toplevel(self.window)
        self.top_interactive_figure.geometry("1800x1100")
        self.top_interactive_figure.resizable(False, False)
        ...
        video = cv2.VideoCapture(movieFile)
        self.sampledFps = video.get(cv2.CAP_PROP_FPS)
        timeStamps = [i * 1/self.sampledFps for i in range(len(y_pred))]
        # plot the new coordinate system
        video.set(1,init_frame); # set up initail frame
        ret, frame = video.read() 
        
            
        video_x = int(video.get(cv2.CAP_PROP_FRAME_WIDTH))
        video_y = int(video.get(cv2.CAP_PROP_FRAME_HEIGHT))
