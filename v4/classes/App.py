...
class tkfunctions:
    def __init__(self):
        ...
        self.regularizationType = '' # default for this version no regularization
        # default to False for this version
        self.pupil_zscore = False
    ...
    def check_individual_file(self):
        ...
        for subjectName in subjects:
            ...
            for movie in movieList:
                ...
                self.event_extracted = False
                self.overall_luminance_extracted = False
                ...
                self.overall_lum_name = "overall_lum.csv"
                if self.overall_lum_name in os.listdir():
                    self.df_overall_lum = pd.read_csv(self.overall_lum_name)
                    if self.df_overall_lum.loc[self.df_overall_lum['movie'] == self.movieName, 'overall_lum'].values[0] >0:
                        self.overall_luminance_extracted = True
                    else:
                        print(f"Overall luminance of movie {self.movieName} not extracted")
                        self.overall_luminance_extracted = False
                else:
                    self.overall_luminance_extracted = False
                    self.df_overall_lum = pd.DataFrame(columns = ['movie', 'overall_lum'])
                    self.df_overall_lum['movie'] = movieList
                    print(f"Overall luminance of movie {self.movieName} not extracted")
                if self.event_extracted and self.overall_luminance_extracted:
                    pass
                else:
    ...
    def buttonFunc_feature_extraction(self):    
        ...
        self.eeObj = eeObj
        if not self.event_extracted:
            eeObj.event_extraction()
        if not self.overall_luminance_extracted:
            luminance_movie = eeObj.overall_lum_extraction()
            # save movie, luminance_movie in the dataframe and save as csv file
            movie_exist = self.df_overall_lum['movie']
            self.df_overall_lum.loc[self.df_overall_lum['movie'] == self.movieName, 'overall_lum'] = luminance_movie
            self.df_overall_lum.to_csv(self.overall_lum_name, index = False)
    ...
    def buttonFunc_modeling(self):
        # pupil size predicted by overall luminance and subject difference (without modeling)
        ...
        if os.path.exists(self.inputDir + "/df_pupil_lum.csv"):
            print("df_pupil_lum.csv already exists. Load the file directly...")
            df_pupil_lum = pd.read_csv(self.inputDir + "/df_pupil_lum.csv")
            self.df_pupil_lum = df_pupil_lum
            ...
            for subjectFolder in os.listdir(self.eyetrackingDir):
                print(subjectFolder)
                subjectDir = self.eyetrackingDir + "\\" + subjectFolder
                ...
                for filename_csv in filename_csvs:
                    ...
                    df_eyetracking = pd.read_csv(subjectDir+ f"\\{filename_csv}", index_col=None, header = None,sep = ",") # Change it to sep = ',' if encounter error
                    ...
                    luminance_movie = self.df_overall_lum.loc[self.df_overall_lum['movie'] == movie, 'overall_lum'].values[0]
            ...
            df_pupil_lum.to_csv(self.inputDir + "/df_pupil_lum.csv", index = False)
        ...
        for subjectName in subjects:
            ...
            if "Eyetracking" in os.listdir(self.inputDir):
                ...
                elif self.modelingType == 'separate':
                ...
                for key in modelList:
                    ...
                    if key in modelResultDict[subjectName].keys():
                        if key == "all":
                            ...
                            #showinfo(title = "", message = f"Subject {subjectName} already modeled with all movies together \n number of weights: {self.nWeight}, map type: {self.mapType}")
                        else:
                            print(f"Subject {subjectName} {key} already modeled")
                            #showinfo(title = "", message = f"Subject {subjectName} {key} already modeled \n number of weights: {self.nWeight}, map type: {self.mapType}")
                        ...
                        if self.modelingType == "all":
                            ...
                            self.exitButton.grid_forget()
                            modeling_progress = tk.Label(self.top_modeling, text = f"Modeling in progress for subject {subjectName} movie {key} \n(number of weights: {self.nWeight}, map type: {self.mapType})",fg = "green")
                            modeling_progress.grid(column = 0, row = 3,columnspan = 2)
    ...
    def plot(self):
        ...
        eeObj.screen_width =self.screen_width
        eeObj.nWeight = self.nWeight
        eeObj.createMapMask(self.mapType)
        ...
        plotObj.df_pupil_lum = self.df_pupil_lum
